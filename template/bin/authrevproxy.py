#!/home/SHARED/software/anaconda3/2020.07b/bin/python
...
class TokenResource(Resource):
    ...
    def getChild(self, path, request):
        ...
        if user == getpass.getuser():
            ...
            try:
                cookie = request.getCookie(b'_ood_token_' + ood_session_id.encode('utf-8')).decode('utf-8')
            except:
                cookie = None
            ...
            try:
                ood_token = os.environ.get("_ood_token_" + ood_session_id.replace("-", "_"))
            except:
                ood_token = None
...
def parse_args(argv=sys.argv[1:]):
    ...
    parser.add_argument('--app-host', dest='app_host', type=str, required=True)
...
def main(argv=sys.argv[1:]):
    ...
    print('authrevproxy is starting on port {} -> {}'.format(args.proxy_port,
                                                             args.app_port) )
    dest = TokenResource(args.app_host, args.app_port, '')
